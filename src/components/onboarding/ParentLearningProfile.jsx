...
export default function ParentLearningProfile({ data, onComplete, onBack }) {
  ...
  const textArea = (key, label, placeholder, hint = '', required = true) => (
    <div>
      <Label className="mb-1.5 block text-sm font-semibold text-gray-700">
        {label}{required && <span className="text-red-500 ml-1">*</span>}
      </Label>
      {hint && <p className="text-xs text-gray-500 mb-2">{hint}</p>}
      <Textarea value={formData[key]} placeholder={placeholder} rows={3}
  ...
  const CheckGrid = ({ label, stateKey, options, cols = 2 }) => (
    <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section>
            ...
            <div className="space-y-5">
              {textArea('learning_success_story', 'Describe a time your child learned something quickly and easily. What made it work?',
                'e.g., When we used Legos to explain fractions, it clicked immediately. She loves building things...')}
              {textArea('learning_struggle_story', 'Describe a time learning was really hard for your child. What caused it?',
          ...
          <section>
            ...
            <div className="space-y-5">
              {textArea('academic_strengths', 'What subjects or skills does your child naturally excel at?',
                'e.g., Math, anything creative, science experiments...', '', false)}
              {textArea('academic_challenges', 'What subjects or skills are the hardest for your child?',
                'e.g., Writing, sitting still for long periods, spelling...', '', false)}
              <CheckGrid label="My child currently struggles with:" stateKey="struggle_areas" options={STRUGGLE_AREAS} cols={2} />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-5">
              {textArea('frustration_response', 'How does your child handle frustration when something is hard?',
                'e.g., She tends to shut down and needs a short break. Once she resets she tries again...')}
              <CheckGrid label="My child generally:" stateKey="behavior_patterns" options={BEHAVIOR_PATTERNS} cols={1} />
              {textArea('additional_notes', 'Anything else about your child\'s personality or needs we should know before Day 1?',
          ...
          <section>
            ...
            <div className="mt-4">
              ...
              <RadioGroup value={formData.homeschooled_before ? 'yes' : 'no'}
                onValueChange={v => setFormData({ ...formData, homeschooled_before: v === 'yes' })}
                className="flex gap-6 mt-2">
                <div className="flex items-center gap-2"><RadioGroupItem value="yes" id="hs-y" /><Label htmlFor="hs-y">Yes</Label></div>
                <div className="flex items-center gap-2"><RadioGroupItem value="no" id="hs-n" /><Label htmlFor="hs-n">No</Label></div>
              </RadioGroup>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all
                ...
                onClick={() => setFormData({ ...formData, biblical_studies: !formData.biblical_studies })}>
                <Checkbox checked={formData.biblical_studies}
                  onCheckedChange={v => setFormData({ ...formData, biblical_studies: v })} />
                <div>
              ...
              <div>
                <Label className="mb-2 block text-sm font-semibold text-gray-700">Character Development Priorities <span className="text-gray-400 font-normal text-xs">(select all that apply)</span></Label>
                  ...
                  {CHARACTER_TRAITS.map(trait => {
                    ...
                    return (
                      <div key={trait}
                        ...
                        onClick={() => {
                          const arr = formData.character_focus;
                          setFormData({ ...formData, character_focus: arr.includes(trait) ? arr.filter(t => t !== trait) : [...arr, trait] });
                        }}>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.key}
                  ...
                  onClick={() => setFormData({
                    ...formData,
                    technology_access: { ...formData.technology_access, [item.key]: !formData.technology_access[item.key] }
                  })}>
                  <Checkbox checked={formData.technology_access[item.key]} />
